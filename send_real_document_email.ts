...
/**
 * Send test email with REAL document data from database
 */
async function sendRealDocumentEmail() {
    console.log('📧 Fetching real document data from database...\n');

    try {
        // Fetch a passport document with all details
        ...
        if (passportDocs.length === 0) {
            console.log('❌ No passport documents found in database.');
            console.log('Please upload a passport document first.\n');
            return;
        }

        const document = passportDocs[0];
    ...
    `;
        ...
        if (result.success) {
            console.log('✅ Email sent successfully with REAL document data!');
            console.log('\n📧 Check your inbox for the notification email.');
            console.log('   Email contains complete document details from database.\n');
        } else {
