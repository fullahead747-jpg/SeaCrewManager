...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/export/fleet-pdf", authenticate, async (req, res) => {
    try {
      ...
      console.error('Fleet PDF export error:', error);
      res.status(500).json({ message: "Failed to generate Fleet PDF export" });
    }
  });
    ...
    try {
      ...
      const html = `
        <div style="font-family: Arial, sans-serif; padding: 20px; color: #333;">
          ...
          <p>Please find attached the latest crew member details for <strong>${vesselName}</strong> in PDF format.</p>
      ...
      if (result.success) {
        res.json({ message: "PDF report emailed successfully" });
      } else {
        res.status(500).json({ message: result.error || "Failed to send email" });
      }
    } catch (error) {
      console.error('Vessel email error:', error);
      res.status(500).json({ message: "Failed to send PDF email" });
    }
  });
  ...
  app.put("/api/documents/:id", authenticate, async (req, res) => {
    try {
      ...
      // CV is an upload-only attachment — skip OCR validation, just update file path (null out all metadata)
    ...
    try {
      ...
      allContracts.forEach(c => {
        if (c.status === 'active') {
          ...
          // Keep the latest-created active contract (matches storage.getCrewMembers behavior)
