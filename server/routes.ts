...
import { generateVesselExcelBuffer } from "./utils/vessel-excel-generator";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Vessel Excel Export & Email
  app.get("/api/vessels/:id/export-excel", authenticate, async (req, res) => {
    try {
      const { buffer, fileName } = await generateVesselExcelBuffer(req.params.id, storage);
      res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
      res.setHeader('Content-Disposition', `attachment; filename=${fileName}`);
      res.send(buffer);
    } catch (error) {
      console.error('Vessel export error:', error);
      res.status(500).json({ message: "Failed to generate Excel export" });
    }
  });

  app.post("/api/vessels/:id/email-excel", authenticate, async (req, res) => {
    try {
      const { additionalEmail } = req.body;
      const { buffer, fileName, vesselName } = await generateVesselExcelBuffer(req.params.id, storage);
      ...
      const html = `
        <div style="font-family: Arial, sans-serif; padding: 20px; color: #333;">
          <h2 style="color: #1e3a5f;">Vessel Crew Report: ${vesselName}</h2>
          <p>Please find attached the latest crew member details for <strong>${vesselName}</strong> in Excel format.</p>
          <hr style="border: none; border-top: 1px solid #eee; margin: 20px 0;" />
          <p style="font-size: 12px; color: #666;">This is an automated report generated by the Crew Management System.</p>
        </div>
      `;

      const result = await smtpEmailService.sendEmailWithAttachment({
        to: recipients.join(','),
        subject: `[REPORT] Vessel Crew Details: ${vesselName} - ${new Date().toLocaleDateString()}`,
        html,
        attachments: [{
          filename: fileName,
          content: buffer,
      ...
      if (result.success) {
        res.json({ message: "Excel report emailed successfully" });
      } else {
        res.status(500).json({ message: result.error || "Failed to send email" });
      }
    } catch (error) {
      console.error('Vessel email error:', error);
      res.status(500).json({ message: "Failed to send Excel email" });
    }
  });
